...
export function ReviewsPage() {
  ...
  return <RiveroPageFrame><main className={styles.lightPage}>
    ...
    <section className={styles.reviewsWall}>{columns.map((column,columnIndex)=><div key={columnIndex}>{columnIndex!==1?<figure data-rivero-reveal="card"><img alt="Rivero customer" src={reviewImages[columnIndex]}/><span>▶</span></figure>:null}{column.map((review,index)=><article data-rivero-reveal key={review.name} style={{"--rivero-delay":`${(index%3)*60}ms`} as CSSProperties}><p>“{review.quote}”</p><footer><img alt="" src={review.avatar}/><span><strong>{review.name}</strong><small>{review.role}</small></span><b>𝕏</b></footer></article>)}{columnIndex!==1?<figure data-rivero-reveal="card"><img alt="Rivero customer" src={reviewImages[columnIndex+1]}/><span>▶</span></figure>:null}</div>)}</section>
    <HowItWorks />
  </main></RiveroPageFrame>;
}
