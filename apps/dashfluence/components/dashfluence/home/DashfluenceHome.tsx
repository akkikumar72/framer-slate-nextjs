...
function ServicesSection() {
  ...
  return <section className={styles.services}><div className={styles.sectionHeadingDark} data-dash-reveal="rise"><SectionEyebrow tone="light">Services</SectionEyebrow><h2>Performance-Driven Services,<br />Built to Convert</h2></div><div className={styles.servicesBody}><div className={styles.serviceVisual} data-dash-reveal="rise"><img alt="Digital marketing team at work" src={dashfluenceAssets.services[active]} /><div><p>{services[active].title}</p><span>{services[active].tagline}</span></div></div><div className={styles.serviceList}>{services.map((service, index) => <article className={`${styles.serviceRow} ${active === index ? styles.serviceActive : ""}`} key={service.slug} onMouseEnter={() => setActive(index)}><Link href={`${DASHFLUENCE_BASE}/services/${service.slug}`} onFocus={() => setActive(index)}><h3>{service.title}</h3><span aria-hidden="true">↗</span></Link><div className={styles.serviceExpanded}><img alt="" src={dashfluenceAssets.services[index]} /><p>{service.copy}</p><small>{service.win}</small></div></article>)}<DashButton href={`${DASHFLUENCE_BASE}/services`} tone="light">See our all services</DashButton></div></div></section>;
}
