...
const brandLogoOrder = [4, 5, 1, 2, 3];

const brandLogos = Array.from({ length: 10 }, (_, index) => {
  const logoNumber = brandLogoOrder[index % brandLogoOrder.length];
  return `/trillo/assets/brand-${logoNumber}.svg`;
});
              ...
              >
                ...
                {[0, 1].map((index) => (
                  <img
                    ...
                    key={index}
                    src={`/trillo/assets/platform-card-${index + 1}.avif`}
                  />
                ))}
              </div>
            </div>
